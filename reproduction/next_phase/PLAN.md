...
| Phase 2 | 完成 | 完整 validation artifacts 均通过验证：两模型各 157,200 rows、96,859 唯一资产—日期、3,144 groups、242 dates、0 failed/nonfinite；TabPFN / FinPFN GPU runtime 1,888.75 / 1,908.48 秒。FinPFN predictive interval width 与绝对 z/rank error 的 pooled Spearman 为 0.2282/0.1048，predictive SD 为 0.2198/0.1042；但二者不能显著预测日期级 IC deterioration，且高 uncertainty 同时有更高 realized-tail precision，存在预测极端程度混杂。 |
| Phase 3 | 完成 | 15 个候选仅用 validation、固定 10 bps 选择。冻结胜出 `rank_buffer_exit20pct`：validation gross/net Sharpe 5.763/1.938，换手 1.624；最佳 uncertainty 候选 `gate_std_75pct` 的 net Sharpe 仅 1.237，低于未修改 FinPFN 的 1.551。唯一一次 test：rank buffer gross/net Sharpe 3.760/-1.040、换手 1.618；未修改 FinPFN 为 4.384/-0.900、1.783。样本外改善未延续，禁止据测试结果重调。 |
| Phase 4 | 完成 | 本地 CPU 13.757 秒。FinPFN 中部 20–80% IC 0.0240，高于 Ridge 0.0153 / LightGBM 0.0187；但 top-40 precision 仅 0.090，低于 0.149 / 0.156，top-40 相邻留存仅 0.084，低于 0.276 / 0.238。支持“广泛小排序改善不能转换为稳定极端选择”的机制。 |
| Phase 5 | 完成 | 建议 B：先做严格预声明的 U.S. released-checkpoint 外部验证，不继续 CSI test 调参或重训。若相同机制重现，则转 C。命令和资源估算已准备，未执行、commit 或 push。 |
