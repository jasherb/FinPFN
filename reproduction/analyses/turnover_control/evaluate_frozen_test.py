...
os.environ.setdefault(
    "MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "finpfn-turnover-test-mpl")
)
...
GATING_DIR = REPOSITORY / "reproduction/runs/turnover_control"
...
def write_report(
    ...
    report = f"""# One-time test evaluation of the frozen configuration

## Result

The validation-frozen **`{selected_name}`** configuration was evaluated once
on 301 common test dates. The cost remains 10 bps per unit of one-way
turnover, and no parameter was changed after observing test results.

The overlay has gross Sharpe {selected_row.gross_long_short_sharpe:.4f}, net
Sharpe {selected_row.net_long_short_sharpe:.4f}, and average total one-way
turnover {selected_row.average_total_one_way_turnover:.4f}. Unmodified FinPFN
has {base_row.gross_long_short_sharpe:.4f},
{base_row.net_long_short_sharpe:.4f}, and
{base_row.average_total_one_way_turnover:.4f}, respectively.

| Comparison | Gross Sharpe | Net Sharpe | Total one-way turnover | Net mean return | Net terminal wealth | Net MDD |
|---|---:|---:|---:|---:|---:|---:|
{chr(10).join(rows)}

## Integrity and interpretation

- `selected_config.json` is marked `frozen_before_test` before test predictions
  are read.
- Unmodified FinPFN, Ridge, and LightGBM reproduce frozen gross Sharpe within
  `1e-10`.
- All models use the same 120,620 asset-dates, raw-return target, deterministic
  tie-break, and actual H-L return-series Sharpe.
- Turnover sums equal-weight long and short one-way turnover and charges initial
  entry from cash.
- The winning validation method contains no uncertainty signal. Uncertainty
  gating was not promoted to test.
- This is the only test evaluation of the pre-frozen strategy.
...
def main() -> None:
    ...
    if any(path.exists() for path in outputs):
        raise FileExistsError(
            "Refusing to overwrite the one-time frozen turnover-control test evaluation"
        )
