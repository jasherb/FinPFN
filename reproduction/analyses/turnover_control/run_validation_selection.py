...
os.environ.setdefault(
    "MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "finpfn-turnover-control-mpl")
)
...
DEFAULT_INPUT = (
    REPOSITORY
    / "reproduction/runs/uncertainty/evaluation/asset_date_uncertainty.parquet"
)
DEFAULT_CONFIG = (
    REPOSITORY
    / "reproduction/analyses/turnover_control/configs/validation_grid.json"
)
DEFAULT_OUTPUT = REPOSITORY / "reproduction/runs/turnover_control"
...
def write_report(
    ...
    report = f"""# Validation-only gating and turnover-control selection

## Selection result

The fixed cost is 10 bps per unit of one-way turnover. All 15 candidates were
declared before results were read and use only 2021 validation. The primary
objective is actual long-short net Sharpe.

The frozen selection is **`{selected['selected_candidate']['name']}`**
(`{selected['selected_candidate']['kind']}`): validation gross Sharpe
{chosen['gross_long_short_sharpe']:.4f}, net Sharpe
{chosen['net_long_short_sharpe']:.4f}, and average total one-way turnover
{chosen['average_total_one_way_turnover']:.4f}. Unmodified FinPFN has gross
Sharpe {baseline.gross_long_short_sharpe:.4f}, net Sharpe
{baseline.net_long_short_sharpe:.4f}, and turnover
{baseline.average_total_one_way_turnover:.4f}.

{markdown_table(['Rank', 'Candidate', 'Type', 'Gross Sharpe', 'Net Sharpe', 'Turnover', 'Coverage', 'Net MDD'], rows)}

## Interpretation boundary

- Full-universe IC/IR is unchanged by a trading overlay.
- Confidence gates retain low-dispersion names within the original tails and
  preserve equal-weight gross exposure.
- Uncertainty-adjusted candidates apply symmetric penalties to long and short
  priority while preventing overlap.
- Rank-buffer and minimum-holding candidates contain no uncertainty signal.
- The selected configuration is frozen before a single test evaluation.

Runtime was {runtime_seconds:.3f} CPU seconds.
"""
...
def main() -> None:
    ...
    gating_root = (REPOSITORY / "reproduction/runs/turnover_control").resolve()
    ...
    if any(path.exists() for path in declared_outputs) or (
        ...
        raise FileExistsError("Refusing to overwrite turnover-control validation outputs")
