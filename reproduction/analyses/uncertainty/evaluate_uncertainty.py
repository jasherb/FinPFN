...
os.environ.setdefault(
    "MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "finpfn-uncertainty-mpl")
)
...
DEFAULT_OUTPUT = REPOSITORY / "reproduction/runs/uncertainty/evaluation"
...
def main() -> None:
    ...
    uncertainty_root = (REPOSITORY / "reproduction/runs/uncertainty").resolve()
    try:
        output_dir.relative_to(uncertainty_root)
    except ValueError as error:
        raise ValueError("Output must stay under reproduction/runs/uncertainty") from error
    outputs = [
