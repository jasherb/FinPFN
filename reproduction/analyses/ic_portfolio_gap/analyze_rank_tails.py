...
os.environ.setdefault(
    "MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "finpfn-ic-gap-mpl")
)
...
OUTPUT = REPOSITORY / "reproduction/runs/ic_portfolio_gap"
DATASET = REPOSITORY / "30features_csi500.parquet"
COMMON_HOLDINGS = (
    REPOSITORY
    / "reproduction/runs/csi500/evaluation/decile_holdings.parquet"
)
FROZEN_METRICS = (
    REPOSITORY
    / "reproduction/runs/csi500/evaluation/portfolio_metrics.csv"
)
FROZEN_IC = (
    REPOSITORY
    / "reproduction/runs/csi500/evaluation/ic_by_period.csv"
)
PREDICTIONS = {
    "FinPFN": (
        REPOSITORY
...
def write_report(
    ...
    report = f"""# FinPFN's IC-portfolio performance gap

## Scope

This analysis reads only 301 frozen test dates, 120,620 common asset-dates,
and one raw-return target. Repeated task rows are averaged by model/date/asset
before ranking. Twenty percentile bins, `k={{10,20,40}}`, and the 20%/80%
local regions were fixed in advance. This is an exploratory mechanism audit,
not feedback for strategy selection.

| Model | Full IC | Middle 20-80% IC | Bottom 20% IC | Top 20% IC | 20-bin monotonicity | Top-40 precision | Bottom-40 precision | Top-40 retention | Bottom-40 retention |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
{chr(10).join(rows)}

## Interpretation

1. **Higher full-universe IC does not imply better tail identification.**
   FinPFN full IC is {finpfn['full_mean_ic']:.4f}, versus
   {ridge['full_mean_ic']:.4f} for Ridge and
   {lightgbm['full_mean_ic']:.4f} for LightGBM, but its top/bottom-40
   precision is {finpfn['top40_precision']:.3f}/
   {finpfn['bottom40_precision']:.3f}.
2. **FinPFN tail membership is less stable.** Adjacent top/bottom-40 retention
   is {finpfn['top40_overlap']:.3f}/{finpfn['bottom40_overlap']:.3f}, versus
   {ridge['top40_overlap']:.3f}/{ridge['bottom40_overlap']:.3f} for Ridge and
   {lightgbm['top40_overlap']:.3f}/{lightgbm['bottom40_overlap']:.3f} for
   LightGBM.
3. **IC and tail return are related but not equivalent.** FinPFN date-level IC
   and same-date H-L return have Spearman correlation
   {finpfn_ic_ls_corr:.3f}. IC uses roughly 401 ranked assets, while the decile
   portfolio uses only about 40 names per leg.
4. **Simple turnover reduction does not automatically preserve signal value.**
   The validation-selected rank buffer reduced test gross/net Sharpe to
   3.760/-1.040.
5. **Repeated 50-stock task sampling is a measurable confounder, not a complete
   explanation.** Mean repetitions per FinPFN asset-date are
   {finpfn['group_composition']['mean_repetitions']:.3f}; composition SD has
   Spearman {finpfn['group_composition']['sd_vs_absolute_rank_error_spearman']:.3f}
   with absolute rank error.

## Largest FinPFN date contributions

| Date | IC | Long-short return |
|---|---:|---:|
{largest_date_rows}

Per-date leave-one-out IC, Sharpe, and return shares are stored in
`date_contributions.csv`. The five largest FinPFN absolute asset contributions
account for {finpfn['asset_absolute_contribution_top5_share']:.2%} of total
absolute contribution.

## Hypothesis assessment

- Broad middle-rank improvement without better extreme selection: supported.
- Less stable FinPFN tail predictions: supported.
- Higher IC from many small ordering improvements: supported.
- Cost erosion from higher turnover: supported, but turnover is not the only
  mechanism.
- Group-wise task construction affects global tails: plausible but not causal
  from the available artifacts.

No sector, size, volatility, or liquidity subgroup is inferred from ambiguous
feature names. Runtime was {runtime_seconds:.3f} CPU seconds.
"""
...
def main() -> None:
    ...
    if any(path.exists() for path in declared) or (
        ...
        raise FileExistsError("Refusing to overwrite IC/portfolio-gap outputs")
